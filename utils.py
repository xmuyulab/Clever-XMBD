...
def read_fcs(
    ...
    Args:
        ...
        meta_path: Path to the obs data in AnnData.
    ...
    if meta_path is not None:
        ...
        obs_m = pd.DataFrame(index=data.index)
    ...
    if transform:
        data[marker_cols] = data[marker_cols].apply(lambda x: np.arcsinh(x / 5)) # cofactor=5
